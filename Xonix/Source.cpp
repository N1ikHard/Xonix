...
struct Enemy 
{
	...
	void move() {
		x += dx;
		//Смена направления при ударе о стену
		if (grid[y / ts][x / ts] == 1) 
		{
			dx = -dx; x += dx;
		}
...
//Сбросить значение координат. Если поле пустое , то обновляется значение на пустоту, либо стену строка 159-163
...
int main()
{
	...
	while (app.isOpen()) {
	   ...
	   //Передвижение противника
		...
		//Проверка координат поля на стену и пустоту.
		for (int i = 0; i < M; i++)
			for (int j = 0; j < N; j++)
				if (grid[i][j] == -1)
					grid[i][j] = 0;
